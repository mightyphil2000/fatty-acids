...
Dat1[which(Dat1$Nstudies<5),c("Nstudies","rsid","lnor","chr")]
Dat1[Dat1$rsid == "",]
write.table(Dat1[which(Dat1$Nstudies<6),c("Nstudies","rsid")],"~/prox_crc_64.txt",sep="\t",col.names=T,row.names=F,quote=F)
Dat1[Dat1$lnor>0.4,c("Nstudies","rsid")]
dat<-Dat1
...
png("~/fatty-acids/outcome_data/results/plots/predicted_lnor/gecco64.png",width=500,height=500)
	ggplot2::ggplot(dat) + ggplot2::geom_point(ggplot2::aes(x=X, y=Y,colour=Nstudies2))+ ggplot2::ggtitle(Title) +ggplot2::labs(y= Ylab, x =Xlab,subtitle=subtitle) + ggplot2::theme(plot.title = ggplot2::element_text(size = Title_size, face = "plain"))+
		...
		ggplot2::scale_colour_manual(name = "Nstudies in\nSNP analysis",
...
Xlab<-"N studies"
Ylab<-"Absolute bias"
png("~/fatty-acids/outcome_data/results/plots/predicted_lnor/gecco64_bias.png",width=500,height=500)
	ggplot2::ggplot(dat) + ggplot2::geom_point(ggplot2::aes(x=X, y=Y,colour=Nstudies2))+ ggplot2::ggtitle(Title) +ggplot2::labs(y= Ylab, x =Xlab,subtitle=subtitle) + ggplot2::theme(plot.title = ggplot2::element_text(size = Title_size, face = "plain"))+
				ggplot2::theme(axis.title=ggplot2::element_text(size=Title_xaxis_size))+
