...
Nstudies2[Nstudies==3]<-"3 studies"
Nstudies2[Nstudies<3]<-"<3 studies"
table(Nstudies2)
dat$Y<-dat$lnor_sh
dat$X<-dat$lnor
Model1<-summary(lm(Y~X,dat[Nstudies2=="3 studies",]))
...
subtitle<-paste0("intercept=",round(int1,3)," | ","slope=",round(slope1,3)," (SNPs present in 3 studies)","\nintercept=",round(int2,3)," | ","slope=",round(slope2,3)," (all SNPs)")
...
png("~/fatty-acids/outcome_data/results/plots/predicted_lnor/hnmsc70.png",width=500,height=500)
...
Xlab<-"N studies"
Ylab<-"Absolute bias"
png("~/fatty-acids/outcome_data/results/plots/predicted_lnor/hnmsc_70_bias.png",width=500,height=500)
	ggplot2::ggplot(dat) + ggplot2::geom_point(ggplot2::aes(x=X, y=Y,colour=Nstudies2))+ ggplot2::ggtitle(Title) +ggplot2::labs(y= Ylab, x =Xlab,subtitle=subtitle) + ggplot2::theme(plot.title = ggplot2::element_text(size = Title_size, face = "plain"))+
				ggplot2::theme(axis.title=ggplot2::element_text(size=Title_xaxis_size))+
		ggplot2::scale_colour_manual(name = "Nstudies that contributed\nto each SNP",
		labels = c("<3 studies","3 studies"),
