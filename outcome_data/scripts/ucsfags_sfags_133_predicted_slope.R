...
mac_case2<-as.character(mac_case)
mac_case2[mac_case>=100]<-">100"
mac_case2[mac_case<100 & mac_case>=50]<-"50-100"
mac_case2[mac_case<50]<-"<50"

mac_control2<-as.character(mac_control)
mac_control2[mac_control>=100]<-">100"
mac_control2[mac_control<100 & mac_control>=50]<-"50-100"
mac_control2[mac_control<50]<-"<50"
...
Colour<-rep(NA,length(mac_case2))
Colour[mac_case2 == "<50" | mac_control2 == "<50"]<-"red"
Colour[mac_case2 == "≥50" | mac_control2 == "≥50"]<-"black"
table(Colour)
dat$Y<-dat$lnor_sh
dat$X<-dat$lnor
...
Model1<-summary(lm(Y~X,dat[mac_case>=50 | mac_control>=50,]))
...
subtitle<-paste0("intercept=",round(int1,3)," | ","slope=",round(slope1,3)," (SNPs with imputation info/r2>0.90)","\nintercept=",round(int2,3)," | ","slope=",round(slope2,3)," (All SNPs)")
...
png("~/fatty-acids/outcome_data/results/plots/predicted_lnor/nb_ugc_104.png",width=500,height=500)
