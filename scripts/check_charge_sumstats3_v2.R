...
File<-"~/MR_FattyAcids/paper/marker_paper/famrc_paper_draft3/response_to_reviewers_round2/data/N6meta2041.tbl.gz.tab"
...
puf<-data.table::fread(File)
puf1<-puf[puf$snp %in% snplist,]

Dat<-format_data(dat=puf1,trait="arachidonic acid",population="European",ncontrol="n",rsid="snp",effect_allele="effect_allele",other_allele="other_allele",beta="beta",se="se",eaf="effect_allele_freq",p="p")

Plot1<-make_plot_maf(ref_1000G="EUR",target_dat=Dat,Title_size=10,Title_xaxis_size=10,nocolour=TRUE)
Plot2<-make_plot_maf(ref_1000G=c("AFR","AMR","EAS","EUR","SAS","ALL"),target_dat=Dat,Title_size=10,Title_xaxis_size=8,nocolour=TRUE,legend=FALSE)
Plot3<-make_plot_gwas_catalog(dat=Dat,trait="Plasma omega-6 polyunsaturated fatty acid levels (arachidonic acid)",beta="beta",se="se",Title_size=10,Title_xaxis_size=10,force_all_trait_study_hits=TRUE,nocolour=TRUE)
...
combine_plots(Plot_list=Plot_list,out_file="~/MR_FattyAcids/paper/marker_paper/famrc_paper_draft3/response_to_reviewers_round2/Figure3_nocolour.tiff")
