...
cancer_data2<-cancer_data1[cancer_data1$trait=="Lung cancer" & cancer_data1$study == "ILCCO/UKB",]
...
fads1_data<-format_results(study="ILCCO/UKB")

data_list<-prep_data(Dat=fads2)
Dat<-data.frame(data_list[1],stringsAsFactors=F)
ref_dat<-data.frame(data_list[2],stringsAsFactors=F)
Dat2<-harmonise_dat(Dat=Dat,ref_dat=ref_dat,marker="marker")
fads2_data<-format_results(study="ILCCO/UKB")
...
save(Dat,file="~/fatty-acids/mr/data/data_for_cismvmr_lung_cancer_ilcco_ukb.RData")
