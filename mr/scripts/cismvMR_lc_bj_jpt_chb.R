...
# chb+jpt ("north east Asian") ld reference panel from 1000 genomes n=207
...
load("data/data_for_cismvmr_lung_cancer_bj.RData")
sort(unique(Dat$trait))
head(Dat)
length(which(!unique(Dat$marker) %in% fads_frq$SNP))
...
Res$panel<-"JPT+CHB"
write.table(Res,"~/fatty-acids/mr/results/cismvMR_lc_bj_jpt_chb_results.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
