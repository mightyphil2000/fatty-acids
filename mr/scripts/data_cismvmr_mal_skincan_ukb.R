...
source("~/fatty-acids/mr/scripts/data_cismvmr_functions.R")
...
sd(fa.tab1$beta[fa.tab1$trait == "AA:DGLA" & fa.tab1$study == "CHARGE"],na.r=TRUE)
sd(fa.tab1$beta[fa.tab1$trait == "GLA:LA" & fa.tab1$study == "CHARGE"],na.rm=TRUE)

tissues<-unique(gtex_data1$tissue)
tissues<-tissues[!tissues %in% c( "esophagus gastroesophageal junction", "esophagus mucosa", "esophagus muscularis",    "lung","artery coronary","colon sigmoid","colon transverse")]

fads1<-format_data3(lun_data=NULL,crc_data=NULL,cancer_data=cancer_data2,gtex_data=gtex_data1,fa.tab=fa.tab1[fa.tab1$trait %in% c("GLA:LA","AA:DGLA"),],ref=ref1,eqtlgen_data=eqtlgen_data1,gene_tab=gene_tab1,ld_eas_pop=NULL,gene="FADS1",region=500000,gtex_tissues=tissues,gwis_file=gwis_file1,studies="CHARGE",fix_log10=FALSE,bbj_eqtl_data=NULL,pre_formatted_data=TRUE) #fix_log10 rescales the Z scores when -log10Pvalue is greater than 1000. Should only be needed for the regional association plots 
fads2<-format_data3(lun_data=NULL,crc_data=NULL,cancer_data=cancer_data2,gtex_data=gtex_data1,fa.tab=fa.tab1[fa.tab1$trait %in% c("GLA:LA","AA:DGLA"),],ref=ref1,eqtlgen_data=eqtlgen_data1,gene_tab=gene_tab1,ld_eas_pop=NULL,gene="FADS2",region=500000,gtex_tissues=tissues,gwis_file=gwis_file1,studies="CHARGE",fix_log10=FALSE,bbj_eqtl_data=NULL,pre_formatted_data=TRUE) 
...
fads1_data<-format_results(study="UKB")

data_list<-prep_data(Dat=fads2)
Dat<-data.frame(data_list[1],stringsAsFactors=F)
ref_dat<-data.frame(data_list[2],stringsAsFactors=F)
Dat2<-harmonise_dat(Dat=Dat,ref_dat=ref_dat,marker="marker")
fads2_data<-format_results(study="UKB")
...
save(Dat,file="~/fatty-acids/mr/data/data_for_cismvmr_malignant_skin_cancer_ukb.RData")
load("~/fatty-acids/mr/data/data_for_cismvmr_malignant_skin_cancer_ukb.RData")
...
format_results<-function(study=NULL){
	Dat3<-rbind.fill(ref_dat,Dat2)
	Dat3$study[is.na(Dat3$study)]<-study
