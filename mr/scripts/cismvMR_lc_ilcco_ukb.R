...
load("data/data_for_cismvmr_lung_cancer_ilcco_ukb.RData")
...
exposure1<-c("AA:DGLA / D5D","FADS1 expression in adipose subcutaneous","FADS1 expression in adipose visceral omentum","FADS1 expression in blood","FADS1 expression in whole blood","FADS1 expression in lung","FADS1 expression in liver")
exposure2<-c("GLA:LA / D6D","FADS2 expression in adipose subcutaneous","FADS2 expression in adipose visceral omentum","FADS2 expression in blood","FADS2 expression in whole blood","FADS2 expression in lung","FADS2 expression in liver")
...
Res$outcome<-"Lung cancer"
Res$panel<-"UKB"

write.table(Res,"~/fatty-acids/mr/results/cismvMR_lc_ilcco_ukb_results.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
...
for (j in 1:7) {
	...
	if (j == 6) { D5D <- Dat[Dat$trait=="FADS1 expression in lung",];            D6D <- Dat[Dat$trait=="FADS2 expression in lung",] }
	if (j == 7) { D5D <- Dat[Dat$trait=="FADS1 expression in liver",];                    D6D <- Dat[Dat$trait=="FADS2 expression in liver",] }

	outcome <- Dat[Dat$trait=="Lung cancer",]
