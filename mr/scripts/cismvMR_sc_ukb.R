...
load("data/data_for_cismvmr_malignant_skin_cancer_ukb.RData")
# standardise GLA:LA beta
...
exposure1<-c("AA:DGLA / D5D","FADS1 expression in adipose subcutaneous","FADS1 expression in adipose visceral omentum","FADS1 expression in blood","FADS1 expression in whole blood","FADS1 expression in liver","FADS1 expression in skin not sun exposed suprapubic","FADS1 expression in skin sun exposed lower leg")
exposure2<-c("GLA:LA / D6D","FADS2 expression in adipose subcutaneous","FADS2 expression in adipose visceral omentum","FADS2 expression in blood","FADS2 expression in whole blood","FADS2 expression in liver","FADS2 expression in skin not sun exposed suprapubic","FADS2 expression in skin sun exposed lower leg")
...
Res$panel<-"UKB"
Res$outcome<-"Malignant skin cancer"

write.table(Res,"~/fatty-acids/mr/results/cismvMR_sc_ukb_results.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
...
for (j in 1:8) {
	...
	if (j == 6) { D5D <- Dat[Dat$trait=="FADS1 expression in liver",];                    D6D <- Dat[Dat$trait=="FADS2 expression in liver",] }
	if (j == 7) { D5D <- Dat[Dat$trait=="FADS1 expression in skin not sun exposed suprapubic",]; D6D <- Dat[Dat$trait=="FADS2 expression in skin not sun exposed suprapubic",] }

	if (j == 8) { D5D <- Dat[Dat$trait=="FADS1 expression in skin sun exposed lower leg",]; D6D <- Dat[Dat$trait=="FADS2 expression in skin sun exposed lower leg",] }
	
	outcome <- Dat[Dat$trait=="Malignant skin cancer",]
