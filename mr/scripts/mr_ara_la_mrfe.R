...
# exclude "rs3734398" because it's not an established GWAS hit for arachidonic acid and will confuse the presentation 
dat<-dat[dat$SNP != "rs3734398",]

res<-mr(dat,method_list=c("mr_ivw_fe","mr_wald_ratio"))

res<-subset_on_method(mr_res=res,    multi_snp_method="Inverse variance weighted (fixed effects)")
res$id.analysis<-paste0(res$id.outcome,res$id.exposure)
...
ara$outcome2[duplicated(ara$outcome2)]
...
head(res4)
res4[res4$outcome2=="Overall cancer",]
...
save(res_single2,file="~/fatty-acids/mr/results/res_single_ara_la_fixed_effects_v2.Rdata") #previoous version used random effects but this is inapropriate for ara with only two SNPs, whcih should rather use fixed effects
