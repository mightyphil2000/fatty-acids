...
find_snp_positions<-function(SNP=NULL,bed_format=FALSE,Chr=NULL,Position=NULL){
	...
	if(is.null(SNP)){
		...
		if(length(Position) == 2){
			minbp<-Position[1]-250000
			maxbp<-Position[2]+250000
			snplist<-ref1$V2[ref1$V4> minbp & ref1$V4<maxbp ]
		}
	}
	return(snplist)
...
extract_snps<-function(snplist="/projects/MRC-IEU/users/ph14916/fatty_acids_summary/snplist_Europeans_rsidsonly2.txt",File=NULL,exact_match=FALSE,file_sep="\t",out_dir="/projects/MRC-IEU/users/ph14916/fatty_acids_summary/"){

	if(length(snplist)>1){
		write.table(snplist,paste(out_dir,"temp.txt",sep=""))
		snplist<-paste(out_dir,"temp.txt",sep="")
	}
    if(exact_match){
        system(paste("fgrep -wf ",snplist," ", File," > ",out_dir,"output.txt",sep="")) 
