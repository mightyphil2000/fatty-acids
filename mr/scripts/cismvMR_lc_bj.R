...
fads_ld  = read.table("data/fads_r_matrix_eas.ld", header=FALSE, stringsAsFactors=FALSE)
head(fads_ld)
fads_frq  = read.table("data/fads_r_matrix_eas.frq", header=TRUE, stringsAsFactors=FALSE)
dim(fads_frq)
head(fads_frq)
...
Res$panel<-"EAS"
write.table(Res,"~/fatty-acids/mr/results/cismvMR_lc_bj_eas_results.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
