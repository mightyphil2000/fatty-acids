...
fads1<-format_data3(lun_data=lun_data1,crc_data=NULL,cancer_data=NULL,gtex_data=gtex_data1,fa.tab=fa.tab1[fa.tab1$trait %in% c("GLA:LA","AA:DGLA"),],ref=ref1,eqtlgen_data=eqtlgen_data1,gene_tab=gene_tab1,ld_eas_pop=NULL,gene="FADS1",region=500000,gtex_tissues=tissues,gwis_file=gwis_file1,studies="CHARGE",fix_log10=FALSE,bbj_eqtl_data=NULL,pre_formatted_data=TRUE) #fix_log10 rescales the Z scores when -log10Pvalue is greater than 1000. Should only be needed for the regional association plots 
fads2<-format_data3(lun_data=lun_data1,crc_data=NULL,cancer_data=NULL,gtex_data=gtex_data1,fa.tab=fa.tab1[fa.tab1$trait %in% c("GLA:LA","AA:DGLA"),],ref=ref1,eqtlgen_data=eqtlgen_data1,gene_tab=gene_tab1,ld_eas_pop=NULL,gene="FADS2",region=500000,gtex_tissues=tissues,gwis_file=gwis_file1,studies="CHARGE",fix_log10=FALSE,bbj_eqtl_data=NULL,pre_formatted_data=TRUE) 
...
fads1_data<-format_results(study="TRICL")

data_list<-prep_data(Dat=fads2)
Dat<-data.frame(data_list[1],stringsAsFactors=F)
ref_dat<-data.frame(data_list[2],stringsAsFactors=F)
Dat2<-harmonise_dat(Dat=Dat,ref_dat=ref_dat,marker="marker")
fads2_data<-format_results(study="TRICL")
...
save(Dat,file="~/fatty-acids/mr/data/data_for_cismvmr_lung_cancer_tricl.RData")
load("~/fatty-acids/mr/data/data_for_cismvmr_lung_cancer_tricl.RData")
