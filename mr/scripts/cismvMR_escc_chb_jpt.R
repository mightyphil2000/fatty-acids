...
# chb (Chinese from Beijin) + jpt (japan) ("North East Asian reference panel") from 1000 genomes n=207
fads_ld  = read.table("data/fads_r_matrix_chb_jpt.ld", header=FALSE, stringsAsFactors=FALSE)
head(fads_ld)
fads_frq  = read.table("data/fads_r_matrix_chb_jpt.frq", header=TRUE, stringsAsFactors=FALSE)
dim(fads_frq)
head(fads_frq)
...
load("data/data_for_cismvmr_esophageal_scc.RData")
...
Res$outcome<-"Esophageal squamous cell carcinoma"
Res$panel<-"CHB+JPT"
write.table(Res,"~/fatty-acids/mr/results/cismvMR_escc_chb_jpt_results.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
...
for (j in 1:7) {
	...
	outcome <- Dat[Dat$trait=="Esophageal squamous cell carcinoma",]
