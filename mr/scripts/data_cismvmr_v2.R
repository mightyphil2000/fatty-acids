...
fads1_data<-format_results(drop_palindromic_SNPs=TRUE)

data_list<-prep_data(Dat=fads2)
Dat<-data.frame(data_list[1],stringsAsFactors=F)
ref_dat<-data.frame(data_list[2],stringsAsFactors=F)
Dat2<-harmonise_dat(Dat=Dat,ref_dat=ref_dat,marker="marker")
fads2_data<-format_results(drop_palindromic_SNPs=TRUE)
...
save(Dat,file="~/fatty-acids/mr/data/data_for_cismvmr_excl_palindromic_snps.RData")
